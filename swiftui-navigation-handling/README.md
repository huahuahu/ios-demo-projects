...
- A unified typed `Route` enum for page identity, reused by push, sheet, and full-screen cover.
- Sheet and full-screen-cover state that stores presentation containers instead of separate page enums.
...
- `SwiftUINavigationHandling/NavigationTypes.swift` defines typed routes, tabs, and deep-link values.
...
`Route` says which page is shown. Whether that page is pushed, shown as a sheet, or shown as a cover is decided by the state slot that stores it: a tab path, `Router.sheet`, `Router.fullScreen`, `node.sheet`, or `node.fullScreen`.
